...
EXPECTED_VERSION="${EXPECTED_VERSION:-2.0.6}"
...
REPORT="$REPORT_DIR/RELATORIO-TESTE-${REPORT_SLUG}-${EXPECTED_VERSION}-ADHOC.md"
{
  ...
  echo "- Painel ${EXPECTED_ARCH}: **$([[ "$(value panel-arch.txt)" == *"$EXPECTED_ARCH"* ]] && echo APROVADO || echo REPROVADO)**"
  ...
  echo "- AdsPower ${EXPECTED_ARCH}: **$([[ "$(value adspower-arch.txt)" == *"$EXPECTED_ARCH"* ]] && echo APROVADO || echo REPROVADO)**"
